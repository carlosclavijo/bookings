...
// SearchAvailabilityByDatesByRoomsId returns true if availability exists for roomId, and false if no availabilityu
func (m *postgresDBRepo) SearchAvailabilityByDatesByRoomsId(start, end time.Time, roomId int) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
// GetRestrictionsForRoomByDate return restrictions for a room by date range
